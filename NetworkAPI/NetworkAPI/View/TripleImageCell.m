...
- (void)layoutSubviews {
    ...
    self.leftImg.frame = CGRectMake(self.titleLabel.frame.origin.x, self.titleLabel.frame.origin.y +  self.titleLabel.frame.size.height + MARGIN, self.thumbnailWidth , self.thumbnailHeight);
    self.centerImg.frame = CGRectMake(self.leftImg.frame.origin.x + self.leftImg.frame.size.width + MARGIN ,self.leftImg.frame.origin.y, self.thumbnailWidth , self.thumbnailHeight);
    self.rightImg.frame = CGRectMake(self.centerImg.frame.origin.x + self.centerImg.frame.size.width + MARGIN ,self.leftImg.frame.origin.y, self.thumbnailWidth , self.thumbnailHeight);
    
    self.timestampAndPublisherLable.frame = CGRectMake(self.titleLabel.frame.origin.x, self.leftImg.frame.origin.y + self.leftImg.frame.size.height + MARGIN , self.titleLabel.frame.size.width, [UILabel heightOf1LineWithFrontSize:self.timestampAndPublisherLable]);
...
+ (float)heightOfCellWithTitle:(NSString *)titleLabel timestampAndPublisher:(NSString *) timestampAndPublisher  {
    if ([titleLabel isKindOfClass:NSString.class] && [timestampAndPublisher isKindOfClass:NSString.class] ) {
        ...
        ParentCell.shareInstance.timestampAndPublisherLable.text = timestampAndPublisher;
        ...
        float heightOfTimestampAndPublisher = [UILabel heightOf1LineWithFrontSize:ParentCell.shareInstance.timestampAndPublisherLable];

        float heightOfCell = heightOfTitleLabel + heightOfTimestampAndPublisher + ParentCell.shareInstance.thumbnailHeight + 4 * MARGIN;
