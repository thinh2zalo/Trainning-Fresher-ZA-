...
//        [self.backGroundSettingView addSubview:self.settingView];
        [self.navigationController setNavigationBarHidden:YES];
        [self setUpSettingView];
         UIBarButtonItem *addItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"change_icon"] style:UIBarButtonItemStyleDone target:self action:@selector(settingOsAndVer)];
        [self.navigationItem setLeftBarButtonItem:addItem];
        
    } else {
        [self setupUI];
    }
    self.saveFileBtn.frame = CGRectMake(10 ,self.view.frame.size.height - 100, 60, 60);
...
- (void)moreFunction {
    SCLAlertView *alert = [[SCLAlertView alloc] initWithNewWindow];
        ...
        SCLButton *button = [alert addButton:@"share messenger" target:self selector:@selector(shareMessenger)];
        ...
        [alert showSuccess:@"More Function" subTitle:nil closeButtonTitle:nil duration:0.0f];
    
}


- (void)settingOsAndVer {
...
- (void)setUpSettingView {
    ...
    self.commitBtn.center = CGPointMake(self.settingView.frame.size.width/2, iosBtn.frame.origin.y + 70);
        ...
        self.grRadioBtn.otherButtons = @[iosBtn, androidBtn] ;
        self.grRadioBtn.otherButtons[0].selected = YES;
}
- (void)cancelAlert {
    [_alert fadeOut];
}
...
- (void)saveFile{
    ...
    NSString * path = [NSString stringWithFormat:@"%@/tmp/fileJson.txt", NSHomeDirectory()];
    NSLog(@"path : %@", path);
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [self.view insertSubview:_collectionView belowSubview:self.saveFileBtn];
...
- (UIView *)backGroundSettingView {
    if (!_backGroundSettingView) {
        ...
        [self.view insertSubview:_backGroundSettingView aboveSubview:self.saveFileBtn];
    }
...
- (UIButton *)saveFileBtn {
    if (!_saveFileBtn) {
        _saveFileBtn = UIButton.new;
        _saveFileBtn.backgroundColor = [UIColor blueColor];
        _saveFileBtn.layer.cornerRadius = 30;
        _saveFileBtn.alpha = 0.7;
        [_saveFileBtn addTarget:self action:@selector(saveFile) forControlEvents:UIControlEventTouchUpInside];
        [_saveFileBtn setImage:[UIImage imageNamed:@"save_icon"] forState:UIControlStateNormal];
        [self.view insertSubview:_saveFileBtn aboveSubview:self.collectionView];
    }
    return _saveFileBtn;
