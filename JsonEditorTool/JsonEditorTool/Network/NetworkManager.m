...
- (void)request:(NSURLRequest *)request  completion:(CompletionBlock) completion {
    ...
    NSURLSessionDataTask *dataTask = [self.sessionManager dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        if (error) {
            ...
            id value = [data objectForKey:@"settings"];
             value = [NSNumber numberWithBool:NO];
            id valueOfSetting = [value objectForKey:@"loginFacebook"];
             [valueOfSetting class];
            if ([valueOfSetting isKindOfClass:NSString.class]) {
                NSLog(@"dict");
            } else {
                NSLog(@"not dict");
            }
